...
public class GestionLibroAutor {
	...
	public static void main(String[] args) {
		...
		Integer opcion, listaAutor, cantidad, listaLibro, setCantidad, cantidadAutores, i;
		...
		do {
			...
			case 1:
				...
				crearLibros(vecLibros, crearAutor(vecAutor, cantidadAutores));
				break;
			case 2:
	...
	public static Autor[] crearVectorAutor(Integer vectorLongitud){
		Autor[] vecAutor = new Autor[vectorLongitud];
		return vecAutor;
	}
	...
	public static void crearLibros(Libro[] vecLibros, Integer[] vecIndAutor) {
		int indLibro;
		...
		for (indLibro = 0; indLibro < vecLibros.length; indLibro++) {
			...
			vecLibros[indLibro] = new Libro(titulo, vecIndAutor, precio, cantidad); //autores...
			}
	} 
	
	public static Integer[] crearAutor(Autor[] vecAutor, Integer cantidadAutores) {
		int indVector;
		String nombreAutor, email, genero;
		char generoChar;
		Autor autor;
		Integer[] vecIndAutor = new Integer[cantidadAutores];
		...
		for(indVector=0; indVector < vecIndAutor.length; indVector++) {
			...
			vecAutor[lista-1] = autor;
			vecIndAutor[indVector] = lista - 1;
		}
		return vecIndAutor;
	}
